...
fn fee_vault_set_native_currency_address() {
    ...
    snf::start_prank(CheatTarget::One(fee_vault.contract_address), OWNER());
...
fn fee_vault_deposit_native() {
    ...
    snf::start_prank(CheatTarget::One(erc20.contract_address), SPENDER());
    let fee = starknet::info::get_tx_info().unbox().max_fee.into();
    erc20.approve(fee_vault.contract_address, fee);
    snf::stop_prank(CheatTarget::One(erc20.contract_address));
    snf::start_prank(CheatTarget::One(fee_vault.contract_address), SPENDER());
